...
BEGIN
	...
	SET @SCRIPT = 
	'SELECT  NOME,CPF, IDCLIENTE, ID FROM
		(SELECT ROW_NUMBER() OVER (ORDER BY ' + @ORDER + ') AS Row, NOME,CPF,IDCLIENTE, ID FROM BENEFICIARIOS WITH(NOLOCK))
		AS BENEFICIARIOS WithRowNumbers
